module Gangios
  module Nagios
    @@file = '/home/bbtfr/Downloads/nagios/t/var/status.dat'
    ...
    class Status
      ...
      def parsestatus(statusfile)
        ...
        File.readlines(statusfile).each do |line|
          ...
          # end of a section
          func = "handle_#{handler}".to_sym
          if line =~ /\}/ && handler != "" && self.respond_to?(func, include_private = true)
            send func, blocklines
            handler = ""
          end
        end
      end
